...
**Tech Stack:** bash, git, Ollama API (`http://<퇴역-GPU서버>:11434/v1`)
...
Starts or verifies the remote GPU model on <퇴역-GPU서버>, waits for
...
```yaml
description: Use when user invokes /fiftybox-local or wants the full orchestration pipeline with Qwen3.5 9B (Ollama) fixed for codebase exploration and a local GPU model on <퇴역-GPU서버> for implementation.
```
